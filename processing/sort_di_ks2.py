...
def main():
    ...
    for dataset_id in tqdm(dataset_ids):
        ...
        if os.path.exists(f'/hdd/kampff/raw/Recordings/{dataset_id}/kilosort2'):
            continue
        else:
            ...
            print(f'running KS2 on recording {dataset_id}...')
            t1 = time.time()
            sorting = ss.run_kilosort2(recording=recording,
                                       output_folder=f"/hdd/kampff/raw/Recordings/{dataset_id}/kilosort2",
                                       docker_image=True)
            t2 = time.time()
            print(f'finished running KS2 on recording {dataset_id} in {t2-t1}s.')

            recording_di = si.load_extractor(f"/hdd/kampff/raw/Recordings/{dataset_id}/di")
                    
            print(f'running KS2 on DI recording {dataset_id}...')
            t1 = time.time()
            sorting_di = ss.run_kilosort2(recording=recording_di,
                                          output_folder=f"/hdd/kampff/raw/Recordings/{dataset_id}/kilosort2_di",
                                          docker_image=True)
            t2 = time.time()
            print(f'finished running KS2 on DI recording {dataset_id} in {t2-t1}s.')
